...
def _build_layer_tree(records: list[_Record]) -> list[PsdNode]:
    ...
    for rec in records:
        if rec.section_divider == 3:
            ...
            stack[-1].append(
                PsdGroup(name=rec.name, layer_id=rec.layer_id, children=children)
            )
        else:
            stack[-1].append(
                PsdLayer(
...
def _parse_layer_extra(extra: bytes, is_psb: bool) -> tuple[str, int, int, bool]:
    """Extract `(name, layer_id, section_divider, is_adjustment)` from a record.
