"""Mutation round-trips for CompItem.render_options."""
...
class TestRoundtripRenderOptions:
    def test_classic_shadow_map(self, tmp_path: Path) -> None:
        ...
        app.project.compositions[
            0
        ].render_options.shadow_map_resolution = ShadowMapResolution.RES_4000

        comp = _reparse(app, tmp_path)

        assert comp.render_options.shadow_map_resolution is ShadowMapResolution.RES_4000

    def test_cinema_4d_quality(self, tmp_path: Path) -> None:
        app = parse_aep(COMPOSITION_DIR / "renderer_cinema_4d.aep")
        app.project.compositions[0].render_options.quality = 99

        assert _reparse(app, tmp_path).render_options.quality == 99

    def test_advanced_scalars(self, tmp_path: Path) -> None:
        app = parse_aep(COMPOSITION_DIR / "renderer_advanced_3d.aep")
        opts = app.project.compositions[0].render_options
        opts.quality = 125
        ...
        assert comp.render_options.quality == 125
        assert comp.render_options.resolution is EnvironmentLightShadowResolution.HALF
        assert comp.render_options.smoothness == 20
    ...
    def test_advanced_casting_box(self, tmp_path: Path) -> None:
        ...
        assert comp.render_options.casting_box_size == pytest.approx(640.0)
    ...
    def test_mapping_write_round_trips(self, tmp_path: Path) -> None:
        app = parse_aep(COMPOSITION_DIR / "renderer_advanced_3d.aep")
        app.project.compositions[0].render_options["Quality"] = 42

        assert _reparse(app, tmp_path).render_options["Quality"] == 42

    def test_bulk_assignment(self, tmp_path: Path) -> None:
        app = parse_aep(COMPOSITION_DIR / "renderer_advanced_3d.aep")
        app.project.compositions[0].render_options = {"Quality": 7, "Smoothness": 2}

        comp = _reparse(app, tmp_path)

        assert comp.render_options.quality == 7
        assert comp.render_options.smoothness == 2
...
class TestRenderOptionsValidation:
    def test_quality_out_of_range(self) -> None:
        ...
        with pytest.raises(ValueError):
            comp.render_options.quality = 126
    ...
    def test_unknown_key(self) -> None:
        ...
        with pytest.raises(KeyError):
            comp.render_options["Nonexistent"] = 1
    ...
    def test_enum_rejects_out_of_range(self) -> None:
        ...
        with pytest.raises(ValueError):
            comp.render_options.shadow_map_resolution = 9
    ...
    def test_casting_box_size_out_of_range(self, bad: float) -> None:
        ...
        with pytest.raises(ValueError):
            comp.render_options.casting_box_size = bad
    ...
    def test_casting_box_center_rejected(self, bad: list[float]) -> None:
        ...
        with pytest.raises(ValueError):
            comp.render_options.casting_box_center = bad
    ...
    def test_smoothness_out_of_range(self) -> None:
        ...
        with pytest.raises(ValueError):
            comp.render_options.smoothness = 21
