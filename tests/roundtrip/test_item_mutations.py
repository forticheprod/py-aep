...
class TestAddComp:
    ...
    def test_add_comp_inserts_at_front(self) -> None:
        # AE 2026 places a new comp at the FRONT of the root folder, right
        # after the `fdta` header (verified via headless script addComp).
        app = parse_aep(SAMPLES_DIR / "folder" / "folder.aep")
        root = app.project.root_folder
        comp = root.add_comp("NewComp", 1920, 1080, 1.0, 10.0, 24.0)
        container = root._children_container
        # First chunk is the fdta header; the new comp's Item follows it.
        assert container[0].chunk_type == "fdta"
        assert container[1] is comp._item_list
        assert root.items[0] is comp
