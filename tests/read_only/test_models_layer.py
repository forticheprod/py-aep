...
class TestLayerStylesEnabled:
    ...
    def test_can_set_enabled_no_styles(self) -> None:
        """With no styles applied, the group and every style report
        canSetEnabled=False (matching ExtendScript)."""
        layer = get_layer(parse_project(SAMPLES_DIR / "type.aep"), "type_shape")
        layer_styles = next(
            p for p in layer.properties if p.match_name == "ADBE Layer Styles"
        )
        assert isinstance(layer_styles, PropertyGroup)
        assert layer_styles.can_set_enabled is False
        for child in layer_styles.properties:
            assert child.can_set_enabled is False

    def test_can_set_enabled_applied_style(self) -> None:
        """An applied style (and the group) reports canSetEnabled=True;
        phantom styles and Blending Options report False."""
        path = (
            Path(__file__).parent.parent.parent
            / "samples"
            / "models"
            / "selection"
            / "selection_dropshadow_enabled_opacity.aep"
        )
        layer = parse_aep(path).project.compositions[0].layers[0]
        layer_styles = layer["ADBE Layer Styles"]
        assert isinstance(layer_styles, PropertyGroup)
        assert layer_styles.can_set_enabled is True
        for child in layer_styles.properties:
            mn = child.match_name
            if mn == "dropShadow/enabled":
                assert child.can_set_enabled is True
            else:  # phantom styles + Blending Options
                assert child.can_set_enabled is False, mn
